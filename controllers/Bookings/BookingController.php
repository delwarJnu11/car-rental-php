...
class BookingController {
    ...
    function save() {
        if (isset($_POST['create'])) {
            ...
            if ($paid_amount == 0 && $net_payable_amount == $due_amount) {
                $payment_status_id = PaymentStatus::get_status_id("Unpaid");
            } else if ($net_payable_amount >= $due_amount && $paid_amount > 0) {
                $payment_status_id = PaymentStatus::get_status_id("Due");
            } else if ($net_payable_amount == $paid_amount && $due_amount == 0) {
                $payment_status_id = PaymentStatus::get_status_id("Paid");
            }
            ...
            if (!$customer->id) {
                ...
                if ($customer_id) {
                    ...
                    $booking_id = $new_booking->create_booking();

                    $payment = new Payment(null, $booking_id, $customer_id, $net_payable_amount, $paid_amount, $due_amount, "Cash", $payment_status_id, null);
